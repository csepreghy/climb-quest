...
const SECTIONS: { axis: ThemeAxis; label: string; description: string }[] = [
  ...
  { axis: "bg",     label: "Background",     description: "Page background — pastel solids and gradients" },
    ...
    <div className="space-y-6 animate-float-up max-w-3xl">
      ...
      {SECTIONS.map(({ axis, label, description }) => {
        ...
        return (
          <section
            key={axis}
            className="rpg-panel p-4 sm:p-5 space-y-3"
              ...
              {list.map(t => {
                ...
                return (
                  <button
                    ...
                    title={t.name}
                    className={cn(
                      "relative h-12 w-full border-2 transition-all flex items-end justify-center pb-1",
                      isActive
                  ...
                  >
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wider text-white drop-shadow-[0_1px_0_rgba(0,0,0,0.7)]">
                      {t.name}
                    </span>
                  </button>
                );
