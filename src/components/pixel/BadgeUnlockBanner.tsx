...
export function showBadgeUnlock(name: string, image: string, chalk: number) {
  queue.push({ name, image, chalk });
  pump();
}
    ...
    <div className="fixed inset-0 z-[100] grid place-items-center pointer-events-none overflow-hidden">
      ...
      <div className="relative animate-banner-pop text-center px-8 py-6 rounded-xl border border-legendary/40 bg-card/95 shadow-[0_20px_60px_-20px_hsl(35_95%_60%/0.4)]">
        ...
        <div className="mt-3 flex justify-center animate-pop-in">
          <div className="relative h-28 w-28 rounded-full overflow-hidden border-4 border-legendary/60 shadow-[0_0_30px_-8px_hsl(var(--legendary)/0.5)] bg-[hsl(var(--panel-fill))]">
            {data.image ? (
