...
export function LevelsModal({ open, onOpenChange, currentLevel }: { open: boolean; onOpenChange: (v: boolean) => void; currentLevel: number }) {
  return (
          ...
          {LEVELS.map(l => {
            ...
            return (
              <div key={l.level} className={cn("flex items-center gap-3 px-3 py-2.5", cur && "bg-accent/10")}>
                <div className="text-xl w-8 text-center">{l.emoji}</div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold flex items-center gap-2">
                    Lv {l.level} · {l.title}
                    {cur && <span className="text-[10px] uppercase px-1.5 rounded bg-accent/20 text-accent">Current</span>}
                  </div>
                  <div className="text-[11px] text-muted-foreground truncate">{l.unlocks.slice(0, 3).join(" · ")}</div>
                </div>
