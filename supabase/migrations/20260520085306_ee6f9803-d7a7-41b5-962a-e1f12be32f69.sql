...
CREATE FUNCTION public.get_admin_users()
 RETURNS TABLE(user_id uuid, email text, character_name text, display_name text, is_admin boolean, level integer, total_chalk_earned bigint, total_logs integer, bosses_sent integer, created_at timestamp with time zone, provider text)
 LANGUAGE plpgsql
